...
我们考虑下面这个看似正确的表达式：
$$
\begin{align}&\lim_{x\rightarrow 0}\lim_{y\rightarrow 0}\frac{x^2}{x^2+y^2}=\lim_{y\rightarrow 0}\lim_{x\rightarrow 0}\frac{x^2}{x^2+y^2}&(1.2)\end{align}
$$
因为我们有
$$
...
$$
其中，记号$x\rightarrow 1^-$表示$x$从$1$的左侧趋向于$1$。当$x$在$1$的左侧时，$\lim\limits_{n\rightarrow \infty}x^n=0$，因此上面等式的左端等于$0$。但是对于任意给定的$n$，我们总可以得到$\lim\limits_{x\rightarrow 1^-}x^n=1$。因此，上面等式右端的极限为$1$。这是否意味着这种类型的极限运算次序交换都是不可信的？<font color=blue>（答案见[命题14.3.3](..\..\第14章\md\实分析 14.3 一致收敛性与连续性.md)）</font>
...
$$
当$y\rightarrow \infty$时取极限，我们可以得到：
$$
\int^{+\infty}_{-\infty}\lim_{y\rightarrow \infty}\frac{\text dx}{1+(x-y)^2}=\lim_{y\rightarrow \infty}\int^{+\infty}_{-\infty}\frac{\text dx}{1+(x-y)^2}=\pi
$$
但是对于任意$x$，我们有$\displaystyle \lim_{y\rightarrow \infty}\frac{\text dx}{1+(x-y)^2}=0$。因此，我们似乎可以推导出$0=\pi$。上述论证出现了什么问题？是否应该舍弃（非常有用的）交换极限运算与积分运算次序的技巧？<font color=blue>（[定理14.6.1](..\..\第14章\md\实分析 14.6 一致收敛和积分.md)给出了部分答案）</font>
...
$$
但是，因为$\displaystyle\lim_{x\rightarrow 0}\frac{x}{1+x}=\frac{0}{1+0}=0$，所以这个结果是不正确的。显然，只有当$x\rightarrow x_0$，$f(x)$和$g(x)$均趋向于$0$时，洛必达法则才适用，而上面的这个例子却没有满足该条件。但是即便当$x\rightarrow x_0$，$f(x)$和$g(x)$均趋向于$0$时，仍然存在出现错误结果的可能。例如，考虑如下极限：
$$
\lim_{x\rightarrow 0}\frac{x^2\sin(x^{-4})}{x}
$$
因此当$x\rightarrow 0$时，分子和分母都趋向于$0$，所以该极限似乎可以放心地使用洛必达发则，从而得到：
$$
\begin{flalign}\lim_{x\rightarrow 0}\frac{x^2\sin(x^{-4})}{x}&=\lim_{x\rightarrow 0}\frac{2x\sin(x^{-4})-4x^{-3}\cos(x^{-4})}{1}\\&=\lim_{x\rightarrow 0}2x\sin(x^{-4})-\lim_{x\rightarrow 0}4x^{-3}\cos(x^{-4})\end{flalign}
$$
根据夹逼定理可知，第一个极限收敛于$0$ <font color=blue>（因为函数$2x\sin(x^{-4})$有上界$2|x|$和下界$-2|x|$，并且当$x\rightarrow 0$时，$2|x|$和$-2|x|$都是趋向于$0$的）</font>，都是第二个极限却是发散的<font color=blue>（因为当$x\rightarrow 0$时，$x^{-3}$趋向于无穷且$\cos(x^{-4})$不趋向于$0$）</font>。因此，极限 $\lim\limits_{x\rightarrow 0}\frac{2x\sin(x^{-4})-4x^{-3}\cos(x^{-4})}{1}$发散。于是，有人可能会根据洛必达法则推出 $\lim\limits_{x\rightarrow 0}\frac{x^2\sin(x^{-4})}{x}$也是发散的。但是我们可以聪明地把这个极限改写成 $\lim\limits_{x\rightarrow 0}x\sin(x^{-4})$，那么根据夹逼定理，当$x\rightarrow 0$时，该极限是趋向于$0$的。这并非说明洛必达法则不可信<font color=blue>（事实上，洛必达法则是非常严格的，见[10.5节](..\..\第10章\md\实分析 10.5 洛必达法则.md)），而是告诉我们在使用它的时候需要更加小心）</font>
