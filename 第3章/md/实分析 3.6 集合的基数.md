...
2. <font color=red>（3.6.1 基数的相等）</font>称两个集合$$X$$与$$Y$$有相同的基数，当且仅当存在一个$$X\rightarrow Y$$的双射$$f:X\rightarrow Y$$。
   ...
   <font color=blue>（注：事实上这个定义无论$X$与$Y$是有限的还是无限的都是有效的，尽管这个时候我们尚且没有无限的定义，至于无限集相关的内容，有兴趣可以移步到[第8章](..\..\第8章\pdf\实分析 8.1 可数性.pdf)学习）</font>
...
7. <font color=red>（习题3.6.10 抽屉原理）</font>设$$A_1$$，……，$$A_n$$都是有限集，且有$$\displaystyle\#(\bigcup_{1\leq i\leq n}A_i)>n$$，则存在$$i\in\{1,\cdots,n\}$$使得$$\#(A_i)\geq 2$$。
...
##### 3.6.3 设$$n$$是一个自然数，且$$f:\{i\in\mathbb N:1\leq i\leq n\}\rightarrow\mathbb N$$是一个函数，证明：存在一个自然数$$M$$使得对任意$$1\leq i\leq n$$，$$f(i)\leq M$$始终成立<font color=blue>（提示：对$$n$$进行归纳，你可能还需要用到一个[引理5.1.14](..\..\第5章\pdf\实分析 5.1 柯西序列.pdf)）</font>。由此我们有对任意自然数集$$\mathbb N$$的有限子集都是有界的。
...
##### 3.6.5 设$$A$$与$$B$$是两个集合，试着构造一个双射证明$$A\times B$$与$$B\times A$$有相同的基数，然后利用命题3.6.14，尝试给出[引理2.3.3](..\..\第2章\pdf\实分析 2.3 乘法.pdf)的另一种证明方法
...
##### 3.6.8 设$$A$$与$$B$$是集合，且存在一个从$$A$$到$$B$$的单射$$f:A\rightarrow B$$（也即集合$$A$$的基数小于或等于集合$$B$$的基数），证明：存在一个从$$B$$到$$A$$的满射$$g:B\rightarrow A$$<font color=blue>（该命题的逆命题证明需要用到选择公理，详情参考[习题8.4.3](..\..\第8章\pdf\实分析 8.4 选择公理.pdf)）</font>
...
[实分析 2.3 乘法](..\..\第2章\pdf\实分析 2.3 乘法.pdf)

[实分析 5.1 柯西序列](..\..\第5章\pdf\实分析 5.1 柯西序列.pdf)

[实分析 8.1 可数性](..\..\第8章\pdf\实分析 8.1 可数性.pdf)
