...
> 首先我们对实数中任何整数$x$都自然成立这个结论（因为整数$x$取它本身作为$N$就可以满足题目结论）。现在我们对任何不是整数的实数$x$做讨论。不妨假设$x={\rm LIM}_{n\rightarrow\infty}a_n$（$(a_n)_{n=1}^\infty$是柯西序列），我们证明：
> $$
> N={\rm LIM}_{n\rightarrow\infty}\left\lfloor a_n\right\rfloor
> $$
...
> > * $(\left\lfloor a_n\right\rfloor)_{n=1}^\infty$是一个柯西序列，并且存在某个整数$M$使得${\rm LIM}_{n\rightarrow\infty}\left\lfloor a_n\right\rfloor=M$。
> > * ${\rm LIM}_{n\rightarrow\infty}\left\lfloor a_n\right\rfloor\leq{\rm LIM}_{n\rightarrow\infty}a_n\leq({\rm LIM}_{n\rightarrow\infty}\left\lfloor a_n\right\rfloor)+1$。
> > * 证明$N$是唯一的。
>
> ---
>
...
> ---
>
> 证明：$(\left\lfloor a_n\right\rfloor)_{n=1}^\infty$是一个柯西序列，并且存在某个整数$M$使得${\rm LIM}_{n\rightarrow\infty}\left\lfloor a_n\right\rfloor=M$：
>
...
>
> 同时，根据前结论，可以得到对任意$\varepsilon>0$，总有整数$N$，$M$使得对任意$n\geq N$有$d(M,\lfloor a_n\rfloor)=d(M,M)=0\leq\varepsilon$，于是可以得到序列$(\left\lfloor a_n\right\rfloor)_{n=1}^\infty$与序列$(M)_{n=1}^\infty$是等价的，即${\rm LIM}_{n\rightarrow\infty}\lfloor a_n\rfloor={\rm LIM}_{n\rightarrow\infty}M=M$，结论得证。
>
> ---
>
> 证明：${\rm LIM}_{n\rightarrow\infty}\lfloor a_n\rfloor\leq{\rm LIM}_{n\rightarrow\infty}a_n\leq({\rm LIM}_{n\rightarrow\infty}\lfloor a_n\rfloor)+1$：
>
> 根据命题5.4.10与命题4.4.1，由对任意有理数$a_n$均有$\lfloor a_n\rfloor\leq a_n<\lfloor a_n\rfloor+1$我们可以得到关系：
> $$
> {\rm LIM}_{n\rightarrow\infty}\lfloor a_n\rfloor\leq{\rm LIM}_{n\rightarrow\infty}a_n\leq{\rm LIM}_{n\rightarrow\infty}(\lfloor a_n\rfloor+1)
> $$
> 又有${\rm LIM}_{n\rightarrow\infty}(\lfloor a_n\rfloor+1)={\rm LIM}_{n\rightarrow\infty}\lfloor a_n\rfloor+1$，于是题式得证。
>
> ---
>
> 证明：$N$是唯一的：
...
> 如果$a_n\leq x$对任意$n\geq 1$成立，那么${\rm LIM}_{n\rightarrow\infty}a_n\leq x$：
>
> 使用反证法。
>
> 我们假设有${\rm LIM}_{n\rightarrow\infty}a_n>x$，那么根据命题5.4.14，存在一个有理数$q$使得${\rm LIM}_{n\rightarrow\infty}a_n>q>x$，那么我们可以由这个结论引申出一个推论：
>
> 由于$q>x\geq a_n$对任意$n\geq 1$成立，所以根据命题5.4.10，$(a_n)_{n=1}^\infty$对应的实数应该小于等于$(q)_{n=1}^\infty$对应实数，即
> $$
> {\rm LIM}_{n\rightarrow\infty}a_n\leq{\rm LIM}_{n\rightarrow\infty}q=q
> $$
> 但是根据我们的前提，又有${\rm LIM}_{n\rightarrow\infty}a_n>q$，于是导出矛盾，反证假设不成立。根据命题5.4.7(a)与定义5.4.6可以得到此时必然有${\rm LIM}_{n\rightarrow\infty}a_n\leq x$成立。
>
> ---
>
> 如果$a_n\geq x$对任意$n\geq 1$成立，那么${\rm LIM}_{n\rightarrow\infty}a_n\geq x$：
>
...
> $$
> {\rm LIM}_{n\rightarrow\infty}a_n\geq{\rm LIM}_{n\rightarrow\infty}q=q
> $$
> 但是根据我们的前提，又有${\rm LIM}_{n\rightarrow\infty}a_n<q$，于是导出矛盾，反证假设不成立。根据命题5.4.7(a)与定义5.4.6可以得到此时必然有${\rm LIM}_{n\rightarrow\infty}a_n\geq x$成立。
