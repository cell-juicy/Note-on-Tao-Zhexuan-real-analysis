...
5. <font color=red>（11.2.14 分段常值积分II）</font>设$I$是一个有界区间，并且设$f:I\to\mathbb R$是$I$上的分段常数函数。那么我们定义$f$的**分段常值积分**$\displaystyle p.c.\int_{I}f$是：
   ...
   <font color=blue>（注：命题11.2.13告诉我们划分的选取是无关紧要的）</font>
...
> 由于命题11.2.13的结论，我们知道分段常数积分的值同划分的选取无关，于是若$f$的分段常数积分是关于$P_1$的，$g$的分段常数积分是关于$P_2$的，我们也能根据命题11.2.13将它们的等效替换为关于$P_1\#P_2$的分段常数积分。故在下面的讨论中，对于涉及到$f$与$g$两个函数的命题我们将在$I$上的分段常数积分统一转变为关于划分$P$的分段常数积分（其中划分$P$是$I$的一个划分，并且满足$f$和$g$都是关于划分$P$的分段常数函数）。
>
...
>    那么$F$是$J$上的分段常数函数，并且$\displaystyle p.c.\int_{J}F=p.c.\int_{I}f$。
>
> 不妨设有$I$的划分$P_I$是使得$f$是关于$P_I$的分段常数函数，然后考虑下面这样一个集合：
> $$
> P_J:=\{A,B\}\cup P_I
> $$
...
> \end{align}
> $$
> 考虑到对任意$S\in P_I$，我们有$S\subseteq I\Longrightarrow F_S=f_S$，于是上式等于$\displaystyle\sum_{S\in P_I}f_{S}|S|$，于是题式得证。
>
> ---
>
> 7. 如果$\{J,K\}$是$I$的一个划分，它将$I$分成两个区间$J$和$K$，那么函数$f|_J:J\to\mathbb R$与$f|_K:K\to\mathbb R$分别是$J$上和$K$上的分段常数函数，并且：
