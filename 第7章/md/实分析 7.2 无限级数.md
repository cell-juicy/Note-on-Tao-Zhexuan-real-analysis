...
2. <font color=red>（7.2.2 级数的收敛）</font>设$\displaystyle\sum_{n=m}^{\infty}a_n$是一个形式无穷级数，对任意的整数$N\geq m$，定义$N$部分和为$\displaystyle S_N:=\sum_{n=m}^{N}a_n$，于是显然$S_N$是一个实数。
           如果当$N\to \infty$时，序列$(S_N)_{N=m}^\infty$__收敛__于某个实数$L$，则称无限级数$\displaystyle\sum_{n=m}^{\infty}a_n$是__收敛__的，并且称它收敛于$L$，也记有$\displaystyle L=\sum_{n=m}^{\infty}a_n$，称$L$是无限级数$\displaystyle\sum_{n=m}^{\infty}a_n$的和。
