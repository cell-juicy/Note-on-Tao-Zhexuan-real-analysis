...
>
> 对任意$\mathbb R^2$中收敛于$(x_0,y_0)$的序列$((x^{(n)},y^{(n)}))_{n=0}^\infty$，我们都有序列$([(x,y)\mapsto x+y](x^{(n)},y^{(n)}))_{n=0}^\infty$收敛于$[(x,y)\mapsto x+y](x_0,y_0)$。从而根据定理13.1.5可以得到加法函数$(x,y)\mapsto x+y$是连续的。
>
...
>> $$
>> I_m:=\{n\in\mathbb N:\exist\:i\in I,i_m=n\}
>> $$
...
##### <font color=blue>（回顾$\displaystyle\limsup_{x\to x_0}f(x):=\inf_{r>0}\sup_{|x-x_0|<r}f(x)$和$\displaystyle\liminf_{x\to x_0}f(x):=\sup_{r>0}\inf_{|x-x_0|<r}f(x)$，这是哪冒出来的）</font>特别地，我们有
