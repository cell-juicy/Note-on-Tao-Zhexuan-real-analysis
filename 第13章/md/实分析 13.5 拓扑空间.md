...
##### 13.5.8 证明：存在一个不可数的良序集$\omega_1+1$，它的最大元素为$\infty$。并且对任意的$y\in\omega_1+1\verb|\|\{\infty\}$，前段$\{x\in\omega_1+1:x<y\}$都是可数的<font color=blue>（提示：利用[习题8.5.19](..\..\第8章\md\实分析 8.5 有序集.md)给实数排序，取全体可数前段的并集，然后添加上最大元素$\infty$）</font>。如果给$\omega_1+1$附加上习题13.5.5中的序拓扑，证明：$\omega_1+1$是紧致的，但是并非所有的序列都存在一个收敛的子序列<font color=blue>（于是我们在度量空间中知道的两个紧致性等价描述在拓扑空间内不是等价的）</font>
...
##### 13.5.9 设$(X,\mathcal{F})$是一个紧致的拓扑空间。假设这个空间是<font color=red>第一可数的</font>，也就是说，对于任意的$x\in X$，存在由$x$的可数个邻域$V_1,V_2,...$构成的邻域簇，使得$x$的任意一个邻域都包含该邻域簇中的一个$V_n$。证明：$X$中的每一个序列都有一个收敛的子序列，从而$X$是序列紧致的<font color=blue>（修改[习题12.5.11](..\..\第12章\md\实分析 12.5 紧致度量空间.md)）</font>，解释这为什么不与习题13.5.8矛盾
...
##### 13.5.10 证明[命题12.2.10](..\..\第12章\md\实分析 12.2 度量空间中的一些点集拓扑知识.md)在拓扑空间中的下述部分类比：(c)蕴含着(a)和(b)，而(a)和(b)是等价的。证明：在习题13.5.7的余可数拓扑空间中，(a)和(b)同时成立，但(c)不成立的情况是有可能发生的
...
##### 13.5.11 设$E$是拓扑空间$(X,\mathcal{F})$的子集。证明：$E$是开的，当且仅当$E$中的每一个元素都是$E$的内点。$E$是闭的，当且仅当$E$包含其全体附着点。证明：[命题12.2.15(e)~(h)](..\..\第12章\md\实分析 12.2 度量空间中的一些点集拓扑知识.md)的类比成立<font color=blue>（其中某些结论可以由定义直接推出）</font>。如果假设$X$是一个豪斯道夫空间，证明：[命题12.2.15(d)](..\..\第12章\md\实分析 12.2 度量空间中的一些点集拓扑知识.md)的类比也成立。举例说明，当$X$不是豪斯道夫空间空间时，(d)是不成立的
...
##### 13.5.13 把[推论12.5.9](..\..\第12章\md\实分析 12.5 紧致度量空间.md)推广到豪斯道夫空间中的紧致集合上
...
##### 13.5.14 把[定理12.5.10](..\..\第12章\md\实分析 12.5 紧致度量空间.md)推广到豪斯道夫空间中的紧致集合上
...
##### 13.5.15 设$(X,d_X)$和$(Y,d_Y)$是两个度量空间<font color=blue>（从而也是拓扑空间）</font>。证明：[定义13.1.1](..\..\第13章\md\实分析 13.1 连续函数.md)和定义13.5.8中函数$f:X\to Y$的连续性概念<font color=blue>（在一点处的连续概念以及在整个定义域上的连续概念）</font>是一致的
...
##### 13.5.16 证明：如果把[定理13.1.4](..\..\第13章\md\实分析 13.1 连续函数.md)推广到拓扑空间，那么(a)就蕴含了(b)<font color=blue>（逆命题不成立，但构造一个反例并不容易）</font>；证明：如果把[定理13.1.5](..\..\第13章\md\实分析 13.1 连续函数.md)推广到拓扑空间，那么(a)、(c)、(d)三者是等价的，它们都蕴含着(b)<font color=blue>（同样，逆向的蕴含关系不成立，但想证明这一点比较困难）</font>
...
##### 13.5.17 把[定理13.3.1](..\..\第13章\md\实分析 13.3 连续性与紧致性.md)和[命题13.3.2](..\..\第13章\md\实分析 13.3 连续性与紧致性.md)推广到拓扑空间中的紧致集合上
